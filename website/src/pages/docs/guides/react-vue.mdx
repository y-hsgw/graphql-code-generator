---
description:
  GraphQL Code Generator is a tool that generates code from your GraphQL schema and operations. It
  can generate TypeScript, Flow, Swift, Kotlin, and more.
---
...
GraphQL Code Generator provides a unified way to get TypeScript types from GraphQL operations for
[most modern GraphQL clients and frameworks](#appendix-ii-compatibility).

This guide is built using the
[Star wars films demo API](https://swapi-graphql.netlify.app/.netlify/functions/index).

We will build a simple GraphQL front-end app using the following Query to fetch the list of Star
Wars films:
...
All the below code examples are available in our
[repository `examples` folder](https://github.com/dotansimha/graphql-code-generator/blob/master/examples).
...
Now, we can start implementing our first query with the `graphql()` function, generated in
`src/gql/`:
...
As showcased
[in one of our recent blog posts](https://www.the-guild.dev/blog/unleash-the-power-of-fragments-with-graphql-codegen),
GraphQL Fragments help build better isolated and reusable UI components.
...
Examples for SWR (React), `graphql-request` and Villus (Vue) are available in our
[repository `examples` folder](https://github.com/dotansimha/graphql-code-generator/blob/master/examples).
...
[GraphQLSP](https://github.com/0no-co/GraphQLSP) package to get:
...
`GraphQLSP`s a TypeScript LSP plugin for GraphQL, to get it working, we need to add the following to
...
[`graphql-eslint`](https://github.com/B2o5T/graphql-eslint) and the
...
```ts
const query: TypedDocumentNode<{ greetings: string }, never | Record<any, never>> = parse(
  /* GraphQL */ `
    query greetings {
      greetings
    }
...
If your stack is not listed above, please refer to other guides ([Angular](/docs/guides/angular),
[Svelte](/docs/guides/svelte)) or to [our plugins directory](/plugins).
